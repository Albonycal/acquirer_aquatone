...
func main() {
	...
	if *sess.Options.SessionPath != "" {
		...
		sess.Out.Important("Loaded Acquirer session at %s\n", *sess.Options.SessionPath)
		...
		f, err := os.OpenFile(sess.GetFilePath("acquirer_report.html"), os.O_RDWR|os.O_CREATE, 0644)
		if err != nil {
	...
	f, _ := os.OpenFile(sess.GetFilePath("acquirer_urls.txt"), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	...
	err = sess.SaveToFile("acquirer_session.json")
	if err != nil {
